...
export default function useCanvas() {
  ...
  const [blockToAddAsGlobal, setBlockToAddAsGlobal] = useState<{
    pageBlockId: string, propName?: string, parentBlockId?: string
  } | null>(null);
  ...
  useEffect(() => {
    ...
    const removeBlock = (blockId: string, propName: string, parentBlockId: string) => {
      const page = activePage;
      if (page) {
        const blocks = page.blocks?.[page.activeLanguageLocale] ?? [];
        ...
        if(blocks.find((block)=>block.id === blockId)){
          newBlocks = blocks.filter((block) => block.id !== blockId);
            ...
            const foundBlock = newBlocks.find((block)=>block.id === parentBlockId)
            if(foundBlock){
              const foundInput = (getValueByPath(foundBlock.inputs, propName?.split('.'))  || []) as PageBlock[];
              foundBlock.inputs = updateValueByPath(foundBlock.inputs, propName.split('.'), foundInput.filter((block)=>block.id != blockId))
            }
    ...
    const handleMessage = async (event: MessageEvent) => {
      ...
      if (data.type === 'addBlock') {
        const { blockId, position, isGlobal, globalBlockId, fromClipBoard, pageBlockId, propName } = JSON.parse(data.content);

        const block = blocks.find((block) => block.Schema.id === blockId);
        if (block) {
          setPageBlocks(block, Number(position), isGlobal, globalBlockId, fromClipBoard || false, pageBlockId, propName);
        }
      } else if (data.type === 'removeBlock') {
        const {pageBlockId, propName, parentBlockId} = JSON.parse(data.content);
        removeBlock(pageBlockId, propName, parentBlockId);
        ...
        if (page) {
          ...
          if(blockIndex < 0){
            ...
            if(foundBlock){
             ...
             const blockInputs = updateValueByPath(foundBlock.inputs, propName.split('.'), foundInput);
             foundBlock.inputs = blockInputs
            }
          }else{
            if (blockIndex === 0) return;
          ...
          if(blockIndex > -1){
            ...
            newBlocks.splice(blockIndex + 1, 0, {
              ...
              if(foundBlock){
                ...
                const newBlocks = [...foundInput].map((block) => ({ ...block, isSelected: false }));
                newBlocks.splice(blockIndex + 1, 0, {
                  ...block,
                  id: uuidv4(),
                });
               const blockInputs = updateValueByPath(foundBlock.inputs, propName.split('.'), newBlocks);
               foundBlock.inputs = blockInputs
              }
          }
        ...
        if (page) {
          ...
          const newBlocks = updateValueByPath(blocks, blockPath ? (`${blockPath}.inputs.${propName}`).split('.')  :  path, value);
          addBlocksToPageHistory(page.activeLanguageLocale, newBlocks);
        } else {
